...
/**
 * @Author: Bob Simon
 * @Description: 游戏主类，继承自JFrame类，负责游戏的全局控制。 内含 1, 一个GameCanvas画布类的实例引用， 2,
 * 一个保存当前活动块(ErsBlock)实例的引用， 3, 一个保存当前控制面板（ControlPanel）实例的引用;
 * @Date: Created in 16:00 2018\5\3
 */
public class ErsBlocksGame extends JFrame {
    ...
    private JMenu mGame = new JMenu("Game"), mControl = new JMenu("Control"),
            mWindowStyle = new JMenu("WindowStyle"), mInfo = new JMenu(
            "Information");
    ...
    miPlay = new JMenuItem("Play"), miPause = new JMenuItem("Pause"),
            miResume = new JMenuItem("Resume"), miStop = new JMenuItem("Stop"),

    miAuthor = new JMenuItem("Author : Bob Simon"),
            miSourceInfo = new JMenuItem(
                    "You can get the source code / document by email");
    ...
    private void createMenu() {
        ...
        miPause.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P,
                KeyEvent.CTRL_MASK));
        ...
        miSetBlockColor.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                Color newFrontColor = JColorChooser.showDialog(
                        ErsBlocksGame.this, "Set color for block", canvas
                                .getBlockColor());
     ...
     * 就继续等待。如果到底了，就看有没有全填满的行， 如果有就删除它，并为游戏者加分，同时随机产生一个 新的当前块，让它自动下落。
     * 当新产生一个块时，先检查画布最顶上的一行是否已经 被占了，如果是，可以判断Game Over了。
     */
    ...
    public static void main(String[] args) {
        new ErsBlocksGame("Russia Blocks designed by Bob Simon");
    }
